...
contract Main is ReentrancyGuard {
    ...
    receive() external payable nonReentrant {
        ...
        try this.failedHeapTest() {
            ...
            require(
                heap.getSize() == heapSizeBefore,
                "Heap should not be modified"
            );
        }
    ...
    function heapTest() public {
        ...
        bytes
            memory data = "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard...";
    ...
    function keccakBoundsTest() public {
        // 135 chars
        bytes
            memory preLimitData = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        // 136 chars
        bytes
            memory limitData = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        // 137 chars
        bytes
            memory overLimitData = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    ...
    function ecrecoverTest() public pure {
        ...
        EcrecoverSignatureTestData memory data5 = EcrecoverSignatureTestData({
            ...
            hash: bytes32(
                uint256(
                    100477317730243874162981143148734960184526648924501832359736878627087072867386
                )
            ),
    ...
    function modexpTests() public view {
        ...
        testCases[0] = ModexpTestCase({
            base: bytes32(
                0x8f3b7d5c187f8abbe0581dab5a37644febd35ea6d4fe3213288f9d63ab82a6b1
            ),
            exp: bytes32(
                0xafa9888e351dfdefd862945b0da33c9ea1de907ae830292438df1fa184447777
            ),
            mod: bytes32(
                0xc7e38934b1501e64e5c0bd0ab35b3354520b6e88b81a1f063c37007c65b7efd5
            ),
            expect: bytes32(
                0x45682b037d21d235bd0ed6103ce2674e5c8e983a88bfd09c847a6324e77c1ad6
            ),
    ...
    function ecAddTests() public view {
        ...
        testCases[0] = ECAddTestCase({
            p1: [
                bytes32(
                    0x099C07C9DD1107B9C9B0836DA7ECFB7202D10BEA1B8D1E88BC51CA476F23D91D
                ), // x1
                bytes32(
                    0x28351E12F9219537FC8D6CAC7C6444BD7980390D0D3E203FE0D8C1B0D8119950
                ) // y1
            ],
            p2: [
                bytes32(
                    0x21E177A985C3DB8EF1D670629972C007AE90C78FB16E3011DE1D08F5A44CB655
                ), // x2
                bytes32(
                    0x0BD68A7CAA07F6ADBECBF06FB1F09D32B7BED1369A2A58058D1521BEBD8272AC
                ) // y2
            ],
            expect: [
                bytes32(
                    0x25BEBA7AB903D641D77E5801CA4D69A7A581359959C5D2621301DDDAFB145044
                ), // result_x
                bytes32(
                    0x19EE7A5CE8338BBCF4F74C3D3EC79D3635E837CB723EE6A0FA99269E3C6D7E23
                ) // result_y
            ],
            expectedSuccess: true
        });
    ...
    function ecMulTests() public view {
        ...
        testCases[0] = ECMulTestCase({
            p: [
                bytes32(
                    0x1F2A9FD8AB833C4F85ED209B187229ED51C510329CDA700BD1BB6E3483290C4C
                ), // x
                bytes32(
                    0x0F518AE296ED6CF2C9E1449B4AEC256054C8AF11FD339E89377E4037575A156E
                ) // y
            ],
            s: bytes32(
                0x1E2DAB676985FDC3E228CFBCE8AB56BC92F95D354644FAAA56DFC895661AFCAE
            ), // scalar
            expect: [
                bytes32(
                    0x18FB38035EF9A864E189211019D1319170D90F16DA429D564EF71B1F72A45033
                ), // result_x
                bytes32(
                    0x29A541100C87B605110364DB832E9929693132F6E65B9FE1C72EC05075A89D35
                ) // result_y
            ],
            expectedSuccess: true
        });
    ...
    function _ecrecoverOneTest(
        EcrecoverSignatureTestData memory _data
    ) internal pure {
    ...
    function _secp256VerifyTest(
        bytes32[] memory _input,
        bool _expectedSuccess
    ) internal view {
        bytes memory data = abi.encodePacked(_input);

        (bool success, bytes memory _result) = P256_VERIFY_ADDRESS.staticcall(
            data
        );
        ...
        if (_expectedSuccess) {
            ...
            require(
                internalSuccess,
                "P256 Verify should have succeeded, but failed"
            );
        } else {
            require(
                _result.length == 0,
                "P256 Verify should have failed, but succeeded"
            );
        }
    }
    ...
    function _runModexpTestCase(ModexpTestCase memory testCase) internal view {
        ...
        require(
            ...
            "MODEXP precompile call success status mismatch"
        );
    ...
    function _runEcAddTestCase(ECAddTestCase memory testCase) internal view {
        ...
        require(
            success == testCase.expectedSuccess,
            "ECADD precompile call success status mismatch"
        );

        if (success) {
            (bytes32 actualX, bytes32 actualY) = abi.decode(
                result,
                (bytes32, bytes32)
            );
            require(
                actualX == testCase.expect[0] && actualY == testCase.expect[1],
                "ECADD result mismatch"
            );
        }
    }
    ...
    function _runEcMulTestCase(ECMulTestCase memory testCase) internal view {
        ...
        require(
            success == testCase.expectedSuccess,
            "ECMUL precompile call success status mismatch"
        );
        ...
        if (success) {
            ...
            require(
                actualX == testCase.expect[0] && actualY == testCase.expect[1],
                "ECMUL result mismatch"
            );
        }
    }

    function _runEcPairingTestCase(
        ECPairingTestCase memory testCase
    ) internal view {
        bytes memory input;
        require(
            testCase.p1.length == testCase.p2.length,
            "G1 and G2 amounts must match"
        );
        ...
        require(
            success == testCase.expectedSuccess,
            "ECPAIRING precompile call success status mismatch"
        );
    ...
    function assertTValue(uint256 expectedValue) external {
        ...
        require(
            ...
            "The value in transient storage is not what was expected"
        );
    }
    ...
    function queryCodeOracle(
        bytes32 _versionedHash
    ) internal returns (uint256 gasUsed) {
        uint256 gasBefore = gasleft();

        // Call the code oracle
        (bool success, bytes memory returnedBytecode) = CODE_ORACLE_ADDR
            .staticcall(abi.encodePacked(_versionedHash));
        ...
        require(
            Helper.hashL2Bytecode(returnedBytecode) == _versionedHash,
            "Returned bytecode does not match the expected hash"
        );
    }

    function queryCodeOracleNoCopy(
        bytes32 _versionedHash
    ) internal returns (uint256 gasUsed) {
    ...
    function testCodeOracle() public {
        ...
        require(
            gasUsed2 == gasUsed3,
            "Decommitment cost wasnt equal between two calls"
        );
    }
